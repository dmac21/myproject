...
class RegisterForm(Form):
	...
	password=PasswordField(u'请输入密码',validators=[Required(),EqualTo('password2',message='password must match')])
	password2=PasswordField(u'再次确认你的密码',validators=[Required()])
...
class ChangePasswordForm(Form):
    ...
    password2 = PasswordField(u'确认你的新密码', validators=[Required()])
...
class PasswordResetForm(Form):
    ...
    def validate_email(self, field):
        if User.query.filter_by(email=field.data).first() is None:
            raise ValidationError(u'不正确的邮箱地址')
